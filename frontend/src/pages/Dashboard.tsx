...
export default function Dashboard() {
  ...
  const stats = [
    { label: 'Jobs Matched',      value: jobs.length.toString(),         icon: Briefcase,  color: 'text-blue-400',   bg: 'bg-blue-400/10'   },
    { label: 'Applications Sent', value: applications.filter((a:any) => a.status === 'Applied').length.toString(), icon: Send, color: 'text-green-400', bg: 'bg-green-400/10' },
    { label: 'Interviews',        value: interviews.toString(),           icon: BarChart3,  color: 'text-purple-400', bg: 'bg-purple-400/10' },
    { label: 'Response Rate',     value: `${responseRate}%`,              icon: TrendingUp, color: 'text-amber-400',  bg: 'bg-amber-400/10'  },
  ]
      ...
      <div className="mt-6 bg-slate-900 rounded-xl border border-slate-800">
        <div className="flex items-center justify-between p-4 md:p-6 border-b border-slate-800">
          <h2 className="text-white font-semibold">
            {isLoading ? 'Loading Jobs...' : `Top Job Matches (${topJobs.length})`}
          </h2>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12">
            <p className="text-slate-400 text-sm">No jobs found. Go to Jobs page to scrape listings.</p>
          </div>
        ) : (
          <div className="divide-y divide-slate-800">
            {topJobs.map((job) => (
              <div key={job.id} className="flex items-center justify-between p-4 md:p-5 hover:bg-slate-800/50 transition-colors">
                ...
                <div className="flex items-center gap-3 flex-shrink-0 ml-3">
                  ...
                  <span className="text-xs px-2 py-1 rounded-full font-medium hidden sm:block bg-blue-500/20 text-blue-400">
                    New
                  </span>
                </div>
              </div>
